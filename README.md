...
Pour un fournisseur e-mail HTTP compatible, configurez uniquement chez l’hébergeur `EMAIL_OTP_PROVIDER=http`, `EMAIL_OTP_ENDPOINT`, `EMAIL_OTP_API_KEY` et `EMAIL_OTP_FROM`. Pour la vérification téléphone, conservez un fournisseur interchangeable via `PHONE_OTP_CHANNEL` (`sms` ou `whatsapp`) et les identifiants propres au fournisseur. Ne placez jamais ces secrets dans le dépôt, le navigateur ou un journal.
