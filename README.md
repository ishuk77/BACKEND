...
Brevo est aussi pris en charge directement : utilisez `EMAIL_OTP_PROVIDER=brevo`, `BREVO_API_URL=https://api.brevo.com/v3/smtp/email`, `BREVO_API_KEY`, `EMAIL_OTP_FROM` et, si souhaité, `EMAIL_OTP_FROM_NAME`. Les paramètres `OTP_LENGTH=6`, `OTP_EXPIRATION_MINUTES`, `OTP_MAX_ATTEMPTS` et `OTP_RESEND_DELAY_SECONDS` sont appliqués côté serveur. Ne configurez ni clés ni identifiants dans le navigateur ou le dépôt.
